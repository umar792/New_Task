...
const SilderCardEvent = ({ item, index, image }) => {
    return (
        <NavLink
            to={`/tickets/group.details/${item?.id}`}
            className="card_image !w-[100%] !h-[100%] flex flex-col pt-[50px] overflow-hidden relative z-[21] !bg-color1 !text-color2   rounded-lg hover:shadow-md border-[1px] border-color4 "
        >
            <img
                className="rounded-t-lg w-full !mt-[-50px]  object-contain z-[2] duration-200"
...
export default SilderCardEvent