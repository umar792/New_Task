...
module.exports = {
  ...
  createOrdermultiple: async (req, res) => {
    try {
      ...
      // Loop through each order in cartData
      for (const orderData of cartData) {
        ...
        // Loop through each shipped item in the order
        for (const shippedItem of shipped_items) {
          // Extract necessary data from shippedItem
          const {
            items,
            type,
            ship_to_name,
          ...
          const newOrder = await OrderModal.create({
            order_Id: Math.random() * 100 + 20, // Example of generating a random order ID with prefix "ORDER_"
            user: req.user ? req.user._id : "65d9e75a665acec07de6b4e3",
            totalAmount: calculateTotalAmount(items),
            items: items.map(item => ({
              ticket_group_id: item.ticket_group_id,
              splits: parseInt(item.quantity), // Assuming quantity is always a string
              price: item.price,
              name: "Ticket Name Placeholder", // Provide a placeholder value for item name
              // Add any other properties you need for each item
            })),
            type,
            shiptoName: ship_to_name,
            shiptoEmail: email_address_attributes.address,
            payments: payments[0].type, // Assuming there's always only one payment type
            service_fee,
            tax,
            buyer_reference_number,
            external_notes,
          ...
          // Optionally, you can do something with the newOrder
